    ...
    ILogger<NubefactService> logger) : INubefactService
{
    ...
    private object CrearPayloadEmision(Comprobante comprobante)
    {
        ...
        return new
        {
            ...
            cliente_tipo_de_documento = MapTipoDocumento(comprobante.Cliente?.TipoDocumento),
            cliente_numero_de_documento = comprobante.Cliente?.NumeroDocumento ?? string.Empty,
            cliente_denominacion = comprobante.Cliente?.NombreCompleto ?? string.Empty,
            cliente_direccion = comprobante.Direccion,
            cliente_email = string.Empty,
