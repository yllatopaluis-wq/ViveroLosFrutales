...
");
    ...
    {
        migrationBuilder.Sql(@"
IF EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = N'FK_Gasto_Proveedor_ProveedorId')
    ALTER TABLE [erp].[Gasto] DROP CONSTRAINT [FK_Gasto_Proveedor_ProveedorId];
IF EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = N'FK_Ingreso_Cliente_ClienteId')
    ALTER TABLE [erp].[Ingreso] DROP CONSTRAINT [FK_Ingreso_Cliente_ClienteId];
IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_Gasto_EmpresaId_ProveedorId' AND object_id = OBJECT_ID(N'erp.Gasto'))
    DROP INDEX [IX_Gasto_EmpresaId_ProveedorId] ON [erp].[Gasto];
IF EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'IX_Ingreso_EmpresaId_ClienteId' AND object_id = OBJECT_ID(N'erp.Ingreso'))
    DROP INDEX [IX_Ingreso_EmpresaId_ClienteId] ON [erp].[Ingreso];
IF OBJECT_ID(N'erp.Gasto', N'U') IS NOT NULL AND COL_LENGTH(N'erp.Gasto', N'ProveedorId') IS NOT NULL
    ALTER TABLE [erp].[Gasto] DROP COLUMN [ProveedorId];
IF OBJECT_ID(N'erp.Ingreso', N'U') IS NOT NULL AND COL_LENGTH(N'erp.Ingreso', N'ClienteId') IS NOT NULL
    ALTER TABLE [erp].[Ingreso] DROP COLUMN [ClienteId];
");
    }
}